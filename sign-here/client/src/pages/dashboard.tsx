...
export default function Dashboard() {
  ...
  }, [envelopes, searchQuery, statusFilter]);
          ...
          <div className="flex items-center gap-1">
            {STATUS_FILTERS.map((sf) => (
              <Button
                ...
                data-testid={`button-filter-${sf.value}`}
              >
                {sf.label}
              </Button>
            ))}
        ...
        <div>
          <div className="flex items-center justify-between gap-2 mb-2.5">
            <h2 className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">
              {statusFilter === "all" ? "All Envelopes" : `${statusFilter.charAt(0).toUpperCase() + statusFilter.slice(1)} Envelopes`}
            </h2>
            {(searchQuery || statusFilter !== "all") && envelopes && (
              <span className="text-[10px] text-muted-foreground" data-testid="text-result-count">
