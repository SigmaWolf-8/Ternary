...
function Tbl({ headers, rows }: { headers: string[]; rows: string[][] }) {
  return (
    <div className="overflow-x-auto my-4 rounded-lg border border-border">
      <table className="w-full border-collapse text-sm" data-testid="table-isa">
        <thead>
          <tr>
            {headers.map((h, i) => (
              <th key={i} className="text-left text-xs font-semibold uppercase tracking-wider text-primary bg-muted/50 px-4 py-3 border-b-2 border-primary/10">
                {h}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {rows.map((r, ri) => (
            <tr key={ri} className={ri % 2 === 1 ? "bg-muted/30" : ""}>
              {r.map((c, ci) => (
                <td key={ci} className="px-4 py-2.5 border-b border-border text-foreground align-top leading-relaxed">
                  {c}
                </td>
              ))}
            </tr>
...
function Bx({ type, label, children }: { type: "t" | "p" | "n"; label: string; children: React.ReactNode }) {
  const borderColor = type === "t" ? "border-l-amber-500" : type === "p" ? "border-l-emerald-500" : "border-l-primary";
  const labelColor = type === "t" ? "text-amber-500" : type === "p" ? "text-emerald-500" : "text-primary";
  return (
    <div className={`my-4 p-4 rounded-r-lg border-l-[3px] ${borderColor} bg-muted/40`}>
      <div className={`text-[10px] uppercase tracking-[2px] font-bold mb-1 ${labelColor}`}>{label}</div>
...
function Cd({ children }: { children: React.ReactNode }) {
  return <code className="font-mono text-[0.85em] bg-muted px-1.5 py-0.5 rounded border border-border text-primary">{children}</code>;
}

function B({ children }: { children: React.ReactNode }) {
  return <b className="font-bold text-foreground">{children}</b>;
}

function P({ children }: { children: React.ReactNode }) {
  return <p className="mb-3.5 text-muted-foreground leading-relaxed">{children}</p>;
}

function H1({ children }: { children: React.ReactNode }) {
  return <h2 className="text-xl sm:text-2xl font-bold text-foreground mt-9 mb-4 pb-2.5 border-b border-border">{children}</h2>;
}

function H2({ children }: { children: React.ReactNode }) {
  return <h3 className="text-lg sm:text-xl font-semibold text-primary mt-7 mb-3">{children}</h3>;
}

function H3({ children }: { children: React.ReactNode }) {
  return <h4 className="text-base font-semibold italic text-foreground mt-5 mb-2.5">{children}</h4>;
}

function CB({ children }: { children: React.ReactNode }) {
  return <pre className="font-mono text-xs bg-muted/70 text-primary p-4 rounded-lg border border-border overflow-x-auto my-3 leading-relaxed whitespace-pre-wrap">{children}</pre>;
}

function Part({ label, title }: { label: string; title: string }) {
  return (
    <div className="my-12 py-7 px-7 text-center border-y border-primary/20 bg-gradient-to-br from-muted/50 to-transparent rounded-sm">
      <div className="text-xs uppercase tracking-[5px] font-semibold text-amber-500 dark:text-amber-400 mb-1.5">{label}</div>
...
const tabs = [
  ...
  { label: "Instructions", icon: Cpu },
  { label: "Dual-Phase Encryption", icon: Lock },
  { label: "PlenumDB", icon: Database },
  { label: "Ternary Substrate", icon: Binary },
  { label: "Architecture", icon: Layers },
  { label: "Analysis & Compare", icon: Shield },
  { label: "References", icon: FileSignature },
];
...
export default function ISASecurityPaper() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground" data-testid="page-isa-security">
      <div className="max-w-4xl mx-auto px-6">
        ...
        >
          <div className="text-[10px] uppercase tracking-[4px] font-semibold text-primary mb-4">
            Salvi Framework ISA v2.0 {"\u2014"} Technical Paper {"\u2014"} February 2026
          </div>
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-foreground leading-tight mb-2.5" data-testid="text-isa-title">
            ISA-Level Security Primitives<br />for Ternary Computing Architectures
          </h1>
          <p className="text-base sm:text-lg italic text-primary mb-5">
            Capability-Based Access Control, Side-Channel Mitigation,<br />Dual-Phase Encryption, and PlenumDB Encrypted Storage
          </p>
          ...
          <div className="text-xs text-muted-foreground leading-loose">
            <span className="font-semibold text-foreground">Applied Physics Division</span> {"\u2014"} Capomastro Holdings Ltd.<br />
            Alberta, Canada {"\u2014"} PlenumNET Post-Quantum Timing Infrastructure
          </div>
          <div className="flex gap-3 justify-center mt-5 flex-wrap">
            <Button variant="outline" size="sm" asChild data-testid="link-github-repo">
              <a href="https://github.com/SigmaWolf-8/Ternary" target="_blank" rel="noopener noreferrer">
                Repository <ChevronRight className="w-3.5 h-3.5 ml-1" />
              </a>
            </Button>
            <Button variant="outline" size="sm" asChild data-testid="link-plenumnet-platform">
              <a href="https://plenumnet.replit.app" target="_blank" rel="noopener noreferrer">
                PlenumNET Platform <ExternalLink className="w-3.5 h-3.5 ml-1" />
              </a>
            </Button>
          </div>
        </motion.div>
      ...
      <nav className="sticky top-0 z-50 bg-background/90 backdrop-blur-xl border-b border-border" data-testid="nav-isa-tabs">
        <div className="max-w-4xl mx-auto flex overflow-x-auto px-4 scrollbar-hide">
          {tabs.map(({ label, icon: Icon }, i) => (
            <button
              key={i}
              data-testid={`tab-btn-${i}`}
              className={`flex items-center gap-1.5 text-xs uppercase tracking-wider whitespace-nowrap px-3 py-3 border-b-2 transition-colors ${
                tab === i
      ...
      <div className="max-w-4xl mx-auto px-6">
        ...
        {tab === 0 && (
          <motion.div key="tab-0" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }} className="py-2 pb-12" data-testid="panel-overview">
        ...
        {tab === 1 && (
          <motion.div key="tab-1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }} className="py-2 pb-12" data-testid="panel-instructions">
            <H1>The Eight Security Instructions (0x90{"\u2013"}0x97)</H1>
            <P>The Salvi ISA {PLATFORM.VM_ISA_VERSION} contains {PLATFORM.VM_OPCODES} opcodes. Eight of them (opcodes <Cd>0x90</Cd> through <Cd>0x97</Cd>) form the <B>Security and Audit</B> category{"\u2014"}a complete hardware security subsystem that doesn't exist in any other processor architecture.</P>
        ...
        {tab === 2 && (
          <motion.div key="tab-2" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }} className="py-2 pb-12" data-testid="panel-encryption">
...
7. SideChUnmask \u2014 Restore normal execution`}</CB>
        ...
        {tab === 3 && (
          <motion.div key="tab-3" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }} className="py-2 pb-12" data-testid="panel-plenumdb">
...
//   5. AuditLog \u2014 record access with HPTP timestamp`}</CB>
        ...
        {tab === 4 && (
          <motion.div key="tab-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }} className="py-2 pb-12" data-testid="panel-substrate">
            <Part label="Part II" title="Technical Specification" />

            <H1>Abstract</H1>
            <P>This paper presents a unified approach addressing five security domains{"\u2014"}capability-based memory protection, side-channel attack mitigation, constant-time execution guarantees, dual-phase encryption, and native encrypted database storage{"\u2014"}within a coherent set of ISA-level primitives designed natively for a ternary computing architecture. We describe the Security and Audit category (opcodes <Cd>0x90</Cd>{"\u2013"}<Cd>0x97</Cd>) and cryptographic opcodes (<Cd>0x6D</Cd>{"\u2013"}<Cd>0x70</Cd>) of the Salvi Framework's {PLATFORM.VM_OPCODES}-opcode ISA {PLATFORM.VM_ISA_VERSION}, alongside the PlenumDB encrypted PostgreSQL framework.</P>
            <div className="flex flex-wrap gap-1.5 my-3.5">
              {["ternary computing","capability architecture","side-channel masking","ISA security","post-quantum cryptography","bijective ternary logic","dual-phase encryption","PlenumDB","encrypted PostgreSQL","hardware security primitives","constant-time execution","PlenumNET","HPTP","CNSA 2.0","FIPS 140-3"].map((t, i) => (
            ...
            <P>The Salvi Framework implements a complete ternary computing engine: {PLATFORM.VM_OPCODES}-opcode ISA ({PLATFORM.VM_ISA_VERSION}), compiled Rust kernel ({PLATFORM.KERNEL_BINARY_SIZE} ELF binary, {PLATFORM.KERNEL_LOC} LOC across {PLATFORM.KERNEL_SUBSYSTEMS} subsystems), with live deployment via PlenumNET.</P>
        ...
        {tab === 5 && (
          <motion.div key="tab-5" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }} className="py-2 pb-12" data-testid="panel-architecture">
        ...
        {tab === 6 && (
          <motion.div key="tab-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }} className="py-2 pb-12" data-testid="panel-analysis">
        ...
        {tab === 7 && (
          <motion.div key="tab-7" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }} className="py-2 pb-12" data-testid="panel-references">
            ...
            <P>Repository: <a href="https://github.com/SigmaWolf-8/Ternary" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">github.com/SigmaWolf-8/Ternary</a>, HEAD 645001e</P>
            <Tbl
            ...
            <div className="text-sm text-muted-foreground leading-loose">
              <p className="mb-0.5 pl-7 -indent-7">[1] H. M. Levy, <i>Capability-Based Computer Systems</i>, Digital Press, 1984.</p>
              <p className="mb-0.5 pl-7 -indent-7">[2] D. J. Bernstein, "Cache-timing attacks on AES," 2005.</p>
              <p className="mb-0.5 pl-7 -indent-7">[3] Y. Ishai, A. Sahai, D. Wagner, "Private Circuits: Securing Hardware against Probing Attacks," CRYPTO 2003.</p>
              <p className="mb-0.5 pl-7 -indent-7">[3b] Salvi Framework, github.com/SigmaWolf-8/Ternary, 2026.</p>
              <p className="mb-0.5 pl-7 -indent-7">[4] T. Fritzmann et al., "Masked Accelerators for Post-Quantum Cryptography," IACR ePrint 2021/479.</p>
              <p className="mb-0.5 pl-7 -indent-7">[5] E. Rivain, E. Prouff, "Provably Secure Higher-Order Masking of AES," CHES 2010.</p>
              <p className="mb-0.5 pl-7 -indent-7">[6] A. Duc, S. Dziembowski, S. Faust, "Unifying Leakage Models," EUROCRYPT 2014.</p>
              <p className="mb-0.5 pl-7 -indent-7">[7] P. Kocher et al., "Spectre Attacks: Exploiting Speculative Execution," IEEE S&amp;P 2019.</p>
              <p className="mb-0.5 pl-7 -indent-7">[8] NIST, "ML-KEM (FIPS 203), ML-DSA (FIPS 204), SLH-DSA (FIPS 205)," 2024.</p>
              <p className="mb-0.5 pl-7 -indent-7">[9] B. Battistello et al., "Horizontal Side-Channel Attacks and Countermeasures on ISW Masking," CHES 2016.</p>
              <p className="mb-0.5 pl-7 -indent-7">[10] R. N. M. Watson et al., "CHERI: A Hybrid Capability-System Architecture," IEEE S&amp;P 2015.</p>
              <p className="mb-0.5 pl-7 -indent-7">[11] S. Nikova, C. Rechberger, V. Rijmen, "Threshold Implementations Against Side-Channel Attacks," CHES 2006.</p>
              <p className="mb-0.5 pl-7 -indent-7">[12] Arm Ltd., "Morello Programme," 2022.</p>
              <p className="mb-0.5 pl-7 -indent-7">[13] D. E. Knuth, <i>The Art of Computer Programming</i>, Vol. 2, 3rd ed., 1997.</p>
              <p className="mb-0.5 pl-7 -indent-7">[14] ISA Shuffling Extensions, IEEE Trans. CAD, 2023.</p>
              <p className="mb-0.5 pl-7 -indent-7">[15] S. Cassiers, G. Standaert, "Tight Random Probing Security," CRYPTO 2021.</p>
              <p className="mb-0.5 pl-7 -indent-7">[16] J. B. Dennis, E. C. Van Horn, "Programming Semantics for Multiprogrammed Computations," CACM 1966.</p>
              <p className="mb-0.5 pl-7 -indent-7">[17] M. Lipp et al., "Meltdown: Reading Kernel Memory from User Space," USENIX Security 2018.</p>
              <p className="mb-0.5 pl-7 -indent-7">[18] Intel, "Memory Protection Keys," 2016.</p>
              <p className="mb-0.5 pl-7 -indent-7">[19] RISC-V Foundation, "Physical Memory Protection," 2017.</p>
              <p className="mb-0.5 pl-7 -indent-7">[20] N. P. Brusentsov, "Setun: A Ternary Computer," 1958.</p>
              <p className="mb-0.5 pl-7 -indent-7">[21] R. N. M. Watson et al., Cambridge TR-951, 2023.</p>
              <p className="mb-0.5 pl-7 -indent-7">[22] D. W. Jones, "Ternary Number Systems" (unpublished), 2013.</p>
            </div>
          </motion.div>
        )}

        <div className="mt-16 py-6 border-t border-border text-center text-xs text-muted-foreground leading-loose">
      ...
      <Button
        ...
        size="icon"
        className={`fixed bottom-6 right-6 rounded-full shadow-lg z-50 transition-opacity ${showTop ? "opacity-100" : "opacity-0 pointer-events-none"}`}
