...
.SYNOPSIS
    ...
    Builds the node binary, generates 3 TL-DSA (PT26-DSA per TM-2026-016)
    identities, starts a 3-node PlenumNET Array3 cluster, and posts a
    deployment summary to the PlenumNET Node Registry for monitoring.
...
if ($localVersion -eq "unknown") {
    ...
    # This generates throwaway key material — a known limitation.
    try {
        $probeDir = Join-Path ([System.IO.Path]::GetTempPath()) "plenumnet-version-probe"
...
`$neSvcPids = @()
try {
    `$neWmiSvc = Get-WmiObject Win32_Service -Filter "Name='PlenumNET-NinjaExec'" -ErrorAction SilentlyContinue
    if (`$neWmiSvc -and `$neWmiSvc.ProcessId -gt 0) { `$neSvcPids += `$neWmiSvc.ProcessId }
} catch {}
`$neOrphans = Get-Process -Name "ninja-exec" -ErrorAction SilentlyContinue | Where-Object { `$_.Id -notin `$servicePids -and `$_.Id -notin `$neSvcPids }
if (`$neOrphans) {
    foreach (`$p in `$neOrphans) {
        try {
            Stop-Process -Id `$p.Id -Force -ErrorAction Stop
            Add-Content -Path `$wdLog -Value "[`$ts] Killed orphan NinjaExec PID `$(`$p.Id)"
        } catch {
            Add-Content -Path `$wdLog -Value "[`$ts] FAILED to kill NinjaExec orphan PID `$(`$p.Id): `$_"
        }
    }
}
...
# path (e.g. C:\Users\<user>\OneDrive\Desktop). The launcher works from
# either location.
