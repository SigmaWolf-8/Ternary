<# 
  PlenumNET TDNS - Browser Extension Installer v1.0.9
...
Write-Host ""
Write-Host "  PlenumNET TDNS - Browser Extension Installer v1.0.9" -ForegroundColor Yellow
...
function Install-ForBrowser {
    ...
    foreach ($f in $extensionFiles) {
        ...
        try {
            ...
            $wc.Headers.Add("User-Agent", "PlenumNET-Installer/1.0.9")
    ...
    if (Test-Path $manifestPath) {
        ...
        if ($version -ne "1.0.9") {
            Write-Host "  [$BrowserName] WARNING: Expected v1.0.9 but got v$version" -ForegroundColor Red
        }
    }
