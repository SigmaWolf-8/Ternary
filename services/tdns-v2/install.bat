@echo off
title PlenumNET TDNS Extension Installer v1.0.8
echo.
echo   PlenumNET TDNS - Browser Extension Installer v1.0.8
...
powershell -NoProfile -Command "$cb=[DateTimeOffset]::UtcNow.ToUnixTimeSeconds(); [Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12; $wc=New-Object System.Net.WebClient; $wc.Headers.Add('Cache-Control','no-cache'); $wc.Headers.Add('User-Agent','PlenumNET-Installer/1.0.8'); foreach($f in @('manifest.json','background.js','content.js','popup.html','popup.js','dimensions.json','report.html','report.js')){ $wc.DownloadFile('%GH_RAW%/'+$f+'?cb='+$cb, '%INSTALL_DIR%\'+$f) }; foreach($i in @('icon16.png','icon48.png','icon128.png')){ $wc.DownloadFile('%GH_RAW%/icons/'+$i+'?cb='+$cb, '%INSTALL_DIR%\icons\'+$i) }"

set /a count=0
for /r "%INSTALL_DIR%" %%f in (*) do set /a count+=1

rem Verify version
powershell -NoProfile -Command "$m = Get-Content '%INSTALL_DIR%\manifest.json' -Raw | ConvertFrom-Json; Write-Host ('  [%BROWSER_NAME%] Version: ' + $m.version); if ($m.version -ne '1.0.8') { Write-Host '  [%BROWSER_NAME%] WARNING: Expected v1.0.8 but got' $m.version -ForegroundColor Red }"
