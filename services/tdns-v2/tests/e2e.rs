...
// ═══════════════════════════════════════════════════════════════════════════
// Test Fixtures — deterministic scan measurements for spec entities
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
// E2E Test 1: Full Registration → Resolution → Routing Pipeline
// ═══════════════════════════════════════════════════════════════════════════
...
fn e2e_register_resolve_route() {
    ...
    let p_result = crs.register(
        ...
        vec![0x02], pptpro_measurements(), now, Some(100), // HPTP sync
    );
...
fn e2e_fts_heartbeat_to_glb_enforcement() {
    ...
    // Register PPTPro (HPTP-mandatory)
    let p_result = crs.register(
        "pptpro.capomastro.plm".into(), "capomastro.plm".into(),
    ...
    // ── FTS receives heartbeats ──────────────────────────────────
    let mut fts = Fts::new();

    // PPTPro sends good heartbeat
    ...
    // Google (non-HPTP) unaffected
    assert!(glb.check_hptp(&g_addr, now + 1000).is_none());
}

// ═══════════════════════════════════════════════════════════════════════════
// E2E Test 3: CON Tunnel Keys → Wire Packets → Integrity
// ═══════════════════════════════════════════════════════════════════════════
...
fn e2e_con_tunnel_wire_packet() {
    ...
    // Keys should exist (if PPTPro is in Google's neighbor map)
    if let Some(key) = g_out {
        // Verify key is deterministic — rederive and compare
    ...
    assert!(pkt.requires_hptp()); // PPTPro destination = HPTP verify
    ...
    bad_wire[36] ^= 0xFF; // Corrupt payload
    assert!(Packet::from_wire(&bad_wire).is_err());
}

// ═══════════════════════════════════════════════════════════════════════════
// E2E Test 4: Bridge Resolution — .plm vs Legacy
// ═══════════════════════════════════════════════════════════════════════════
...
fn e2e_bridge_resolution() {
    ...
    // ── Unregistered .plm names fail ─────────────────────────────
    match bridge.resolve("nonexistent.plm", now) {
        Resolution::Failed { .. } => {} // Expected
...
// ═══════════════════════════════════════════════════════════════════════════
// E2E Test 5: Property Drift → Redirect → Bridge Re-resolution
// ═══════════════════════════════════════════════════════════════════════════
...
fn e2e_drift_redirect_pipeline() {
    ...
    // ── Google changes: becomes subscription-based ───────────────
    let mut changed = google_measurements();
    changed[19] = RawValue::Pattern("subscription".into()); // trit 20: free → subscription

    let later = now + 1_000_000_000;
    let result = crs.rescan("google.plm", changed, later);

    // Capture drift log length before wrapping in Arc
    ...
    match result {
        Some(VerificationResult::Drifted { old_address, new_address, changed_dims, .. }) => {
            ...
            // ── Bridge resolves to new address ───────────────────
            let crs_shared = std::sync::Arc::new(std::sync::RwLock::new(crs));
            let mut bridge = Bridge::new(crs_shared.clone());
...
// ═══════════════════════════════════════════════════════════════════════════
// E2E Test 6: FTS Death Detection → GLB Reroute
// ═══════════════════════════════════════════════════════════════════════════
...
fn e2e_fts_death_to_glb_reroute() {
    ...
    // Routing to PPTPro should skip it
...
// ═══════════════════════════════════════════════════════════════════════════
// E2E Test 7: SubCube Multicast → Wire → GLB Fan-out
// ═══════════════════════════════════════════════════════════════════════════
...
fn e2e_subcube_multicast_wire() {
    ...
    // PPTPro should be in this sub-cube (trit 16 = 3)
    assert!(sc.contains(&p_addr));
    // Google should NOT (trit 16 = 2)
    assert!(!sc.contains(&g_addr));

    // ── Wire encode multicast packet ─────────────────────────────
    ...
    match glb.forward_subcube(&sc, None, 0) {
        ...
        GlbDecision::DeliverLocal => {
            // Also valid if local node matches
        }
...
// ═══════════════════════════════════════════════════════════════════════════
// E2E Test 8: API Router — Full Registration + Resolution Cycle
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
// E2E Test 9: CON Key Rotation → Tunnel Rekey
// ═══════════════════════════════════════════════════════════════════════════
...
fn e2e_con_key_rotation() {
    ...
    // Second rekey produces different keys again
    con.rekey_all(3000);
