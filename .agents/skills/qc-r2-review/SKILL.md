...
3. **`output_file`** (required): Path where the agent should write its review output.
...
4. **Key provisioning test coverage** — How do you verify correct key generation without exposing private keys? How do you test the negative assertion that tunnel keys are "not stored"? How do you verify that different nodes produce different derived keys? Key generation in PlenumNET uses TLSponge-385 for derivation and TL-KEM for key encapsulation. Test assertions must verify: (a) no raw binary integers enter sponge absorb (INVARIANT 8), (b) context strings match the documented values exactly (context strings are load-bearing), (c) derived keys differ when any input (address, epoch, KEM secret) differs, (d) Rep C output contains no zero trits (zero-sentinel property), (e) signer's Rep C address is bound into the TL-DSA signature context string (INVARIANT 7a), (f) keypair derivation uses the node's Rep C address as domain-separation input to TLSponge-385 alongside the passphrase (INVARIANT 7b), (g) signature verification confirms the signer's public key is registered against a Rep C address (INVARIANT 7c), (h) TL-KEM encapsulation/decapsulation context includes both encapsulator and decapsulator Rep C addresses (INVARIANT 7d), and (i) T-AE-MAC associated data and Phase Encryption context bindings include Rep C addresses of participating nodes (INVARIANT 7e). Distinguish between persistent key material (MasterSecret at the product's encrypted keystore, encrypted at rest via Phase Encryption) and ephemeral topology-derived keys (CON tunnel keys, re-derived on demand from TLSponge-385). The "not stored" assertion applies to tunnel keys only. Acceptable verification methods include: (a) verifying that the public key derived from the generated key matches expected format constraints, (b) performing a sign-then-verify round-trip using only the public key for verification, (c) verifying keystore file permissions and encryption-at-rest properties. Unacceptable methods include: extracting or comparing raw private key material in test assertions, logging key material to test output, or storing generated keys in test fixtures.
