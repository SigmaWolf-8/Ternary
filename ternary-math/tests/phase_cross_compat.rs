...
fn cross_compat_reject_wrong_version() {
    ...
    assert!(matches!(decrypt(&bad3, &key), Err(PhaseError::UnsupportedVersion(3, 4))));
}
...
fn bidirectional_ciphertext_vector() {
    ...
    let decrypted = decrypt(&ingested, &key).unwrap();
...
fn all_modes_ciphertext_stability() {
    ...
    for mode in [
        ...
        let wire2 = ct2.to_ts_wire_format();
        assert_eq!(wire1.primary_data_b64, wire2.primary_data_b64);
        assert_eq!(wire1.secondary_data_b64, wire2.secondary_data_b64);
...
fn cross_compat_kem_end_to_end() {
    ...
    let decrypted = decrypt(&encrypted, &phase_key).unwrap();
    assert_eq!(decrypted, plaintext);
}
