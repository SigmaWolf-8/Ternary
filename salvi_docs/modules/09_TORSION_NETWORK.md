...
- **Generic N-Dimensional Torus** - Configurable from 3D to 13D
- **Preset Configurations** - 7D, 10D, 13D optimized topologies
- **Geodesic Routing** - Shortest-path calculation with torsion weights
- **Route Caching** - LRU cache for frequently-used paths
- **Ternary Addressing** - Native balanced ternary node coordinates
...
│                    Routing Layer                             │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐         │
│  │  Geodesic   │  │   Route     │  │  Torsion    │         │
│  │  Algorithm  │  │   Cache     │  │  Weights    │         │
│  └─────────────┘  └─────────────┘  └─────────────┘         │
├─────────────────────────────────────────────────────────────┤
│                    Topology Layer                            │
│  ┌─────────────────────────────────────────────────────┐   │
...
Each node has `2N` neighbors (+/-1 in each dimension, with wraparound).
...
fn torsion_weight(dimension: usize, direction: Direction) -> f64 {
    ...
    let dir_factor = match direction {
        Direction::Positive => PHI,      // Golden ratio ~ 1.618
...
- [TVM](./11_TVM.md) - Ternary Virtual Machine for computation
- [Cryptography](./05_CRYPTOGRAPHY.md) - Post-quantum security for network traffic
- [BTG](./13_BTG.md) - Binary-ternary conversion for interoperability
