...
**Discrete Isometry · Minimum Distance · Automorphisms · Spectral Theory**
...
This monograph is the definitive formalization of the Representation Universality Thesis for the Salvi Framework. It supersedes TM-2026-004 through TM-2026-007, resolving the critical open problems (chi specification and branch number lower bound) and incorporating computationally verified results that elevate all three proof programs to COMPLETE status for the purpose of security claims.
...
- **GF(3)** = Z₃ = {0,1,2} under addition modulo 3, with multiplication defined by the field structure.
- **Rep C** = {1,2,3} via φ(k) = k+1.
- **ω** = e^(2πi/3) (cube root of unity, used only in the continuous corollary).
- **ω₂₇** = e^(2πi/27) (primitive 27th root, used in spectral theory).
- **GF(27)** = GF(3)[t]/(t³+2t+1).
- **Cyclic distance** on Z_n: δ(a,b) = min(|a−b|, n−|a−b|).
- **L(p; q₁,…,qₙ)** = generalized lens space S^(2n−1)/Z_p.
...
The ternary hypercube Z₃ⁿ has Hamming distance d_H(u,v) = |{i : u_i ≠ v_i}|. The target is Z₂₇ⁿ with ℓ¹ cyclic product metric d₁(a,b) = Σᵢ δ(aᵢ, bᵢ).
...
The multiplier 9 = 3² is the unique value that produces equal spacing of three points in Z₂₇. Since 27/3 = 9, the three images {0, 9, 18} partition Z₂₇ into three arcs of length 9, each corresponding to one trit value.
...
**Corollary 2.4.** δ(9a, 9b) = 0 if a = b, and 9 if a ≠ b. The separation function is binary: trits either agree (distance 0) or disagree (distance 9 = 3²). There is no intermediate value.
...
**Definition 2.6 (Realization map).** The map ρ: Z₂₇ → S¹ is defined by ρ(m) = e^(2πim/27). This is a scaled isometry: arc-length on S¹ between ρ(a) and ρ(b) equals (2π/27) · δ(a, b).
...
**Corollary 2.7 (Continuous scaled isometry).** The composition ι_c = ρⁿ ∘ ι gives d₁(ι_c(u), ι_c(v)) = (2π/3) · d_H(u, v). The scale factor decomposes: (2π/27) × 9 = 2π/3. Standard π enters here and only here — as a property of the realization map that converts discrete positions in Z₂₇ to angles on S¹. It is a characteristic of the bridge, not of the architecture. The ternary geometric constant π = 14, which governs Z₂₈, is unaffected and untouched.
...
*Proof.* A trit flip at position i changes one coordinate of ι(v) by ±9 in Z₂₇. A minimal routing sequence always flips a coordinate where vᵢ ≠ tᵢ, reducing d₁ by 9. Any non-target point has at least one such coordinate, so every non-target point has a neighbor with strictly smaller d₁. ∎
...
**Corollary 4.4.** Each additional distinguished trit value at position i reduces its factor from S₃ (order 6) to its stabilizer (order 2 if one value fixed, order 1 if two values fixed).
...
*Note:* The lens space embedding uses the continuous corollary (ι_c), which routes through the realization map. This is the natural domain for topological invariants since lens spaces are quotients of spheres. The discrete isometry (Theorem 2.5) provides the computational foundation; the continuous corollary provides the topological interpretation.

---

## Part IV: Synthesis

### 5.1 The Factorization Principle

```
Z₃ⁿ  ——ι——→  Z₂₇ⁿ  ——ρⁿ——→  Tⁿ
```

First arrow: discrete isometry, scale 9, purely ternary. Second arrow: realization map, scale 2π/27, where standard π enters. Composition: scale 2π/3. Ternary π = 14 governs Z₂₈ independently.
...
| 2π/27 | Realization map | Converts Z₂₇ position to S¹ angle | Standard circle circumference |
...
**Not numerology.** Constants 9, 13, 27, 28, 364 are algebraically determined by repunit/cyclotomic/coprimality structure. They form a mutually consistent system where changing any one breaks all others.
...
**Convolution:** For (f\*g)(x) = Σᵧ f(y)g(x−y): (f̂\*g)(k) = f̂(k) · ĝ(k)
...
*Remark.* The discrete torus spectral theory is independent of the continuous geometry; it relies only on the algebraic structure of Z₂₇ⁿ and the existence of complex 27th roots. It constitutes a fully ternary-native framework: domain and frequency indices are both powers of three, and the transform can be implemented with radix-3 FFT algorithms.

---

## Part VI: Remaining Open Problems

### 7.1 Full Semantic Automorphism Catalog
...
### 7.2 Lens Space Topological Invariants
...
|---------|-------------|--------------|----------|
| I: Isometry | Discrete Math / Designs, Codes & Crypto | Discrete isometry | 3–6 months |
| II: Min Distance | IACR ToSC | TIS-27 + chi design | 6–12 months |
| III: Automorphisms | J. Algebra / Topology Appl. | Lens space symmetry | 6–12 months |
| V: Spectral Theory | J. Fourier Analysis / Appl. Comp. Harm. | Ternary DFT framework | 6–12 months |
| Unified | Found. Computational Mathematics | Full synthesis | 12–18 months |

---

## Appendix A: Theorem Index

| Reference | Statement | Status | Section |
|-----------|-----------|--------|---------|
| Lemma 2.3 | Constant discrete separation = 9 | Proved | §2.3 |
| Theorem 2.5 | Discrete scaled isometry (factor 9) | Proved | §2.4 |
...
| Theorem 4.5 | Lens space embedding | Proved | §4.3 |
| Theorem 4.6 | Fundamental group π₁ = Z₃ | Proved | §4.3 |
| Theorem 5.1 | Unified Representation Universality | Proved | §5.2 |
...
Automorphism group corrected from {id}² (TM-004B) to (C₂)² (TM-006+). Chi barrier elevated from assumption to theorem (TM-005 discovery). Discrete isometry primary over continuous (TM-007+). Standard π enters only via realization map ρ.
...
| **HPTP-mandatory** | Addresses with trits 15 and 16 both equal to 2 |
| **Lens space** | Quotient of sphere by cyclic group action |
| **PlenumNET** | Ternary computing architecture |
| **Pontryagin duality** | Dual group of characters of an abelian group |
| **Realization map** | ρ: Z₂₇ → S¹ via ρ(m) = e^(2πim/27) |
| **Rep C** | Representational encoding {1,2,3} |
| **Salvi Framework** | Mathematical foundation of PlenumNET |
| **TIS-27** | Sponge construction with 54-trit state |
| **Wide trail strategy** | Method for proving lower bounds on active S-boxes |
