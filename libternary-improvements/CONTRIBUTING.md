...
The 35-opcode virtual machine uses theory-derived parameters:

| VM Parameter | Value | Derivation |
|-------------|-------|------------|
| Opcode count | 35 | Engineering choice, maps to ternary register width |
...
|-------|-----------|----------------------|
| **Rust Kernel** (`libternary`) | `/libternary`, `/src/kernel` | GF(3) arithmetic, bijective mappings, the 35-opcode specification (ADR-001), `unsafe` Rust and FFI conventions |
...
- If your change alters the behaviour of any of the 35 opcodes, you must update the corresponding `verifyTau()` test case.
...
1. **`verifyTau()` Self-Test** — Executes the τ polynomial identity check across all 35 opcodes. A single failure halts the pipeline.
...
- Formal specification (YAML/JSON) for the 35-opcode instruction set
