...
The 176-opcode virtual machine (ISA v2.1) uses theory-derived parameters:

| VM Parameter | Value | Derivation |
|-------------|-------|------------|
| Opcode count | 176 | ISA v2.1 enterprise-grade expansion (backward compatible with v1.0's 62) |
...
|-------|-----------|----------------------|
| **Rust Kernel** (`libternary`) | `/libternary`, `/src/kernel` | GF(3) arithmetic, bijective mappings, the 176-opcode ISA v2.1 specification (ADR-001), `unsafe` Rust and FFI conventions |
...
│       ├── src/
│       │   ├── vm/       # 176-opcode virtual machine (ISA v2.1)
...
- If your change alters the behaviour of any of the 176 opcodes, you must update the corresponding `verifyTau()` test case.
...
1. **`verifyTau()` Self-Test** — Executes the τ polynomial identity check across all 176 opcodes. A single failure halts the pipeline.
...
- Formal specification (YAML/JSON) for the 176-opcode ISA v2.1 instruction set
