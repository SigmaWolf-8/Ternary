...
impl TritInt {
    ...
    pub fn extended_gcd(a: &TritInt, b: &TritInt) -> (TritInt, TritInt, TritInt) {
        ...
        let ax = a.mul(&old_s);
        let y = if !old_s_neg {
            // gcd = a*x - b*y → y = (a*x - gcd) / b
            if !ax.is_zero() && !old_r.is_zero() {
                let (q, rem) = ax.sub(&old_r).div_mod(b);
                if rem.is_zero() { q } else { TritInt::zero() }
            } else {
                TritInt::zero()
            }
        } else {
            // gcd = -a*x + b*y → y = (gcd + a*x) / b
            let (q, rem) = old_r.add(&ax).div_mod(b);
            if rem.is_zero() { q } else { TritInt::zero() }
        };

        // If x is negative, convert to positive modular representative: x = b - |x|
...
fn signed_add(a: TritInt, a_neg: bool, b: TritInt, b_neg: bool) -> (TritInt, bool) {
    if a_neg == b_neg {
        ...
        if lt_packed(ap, ac, bp, bc) {
            // |b| > |a|: result = |b| - |a|, sign = b's sign
            (b.sub(&a), b_neg)
        } else if lt_packed(bp, bc, ap, ac) {
            // |a| > |b|: result = |a| - |b|, sign = a's sign
            (a.sub(&b), a_neg)
        } else {
            // Equal: result = 0
            (TritInt::zero(), false)
...
mod tests {
    ...
    fn gcd_framework_values() {
        ...
        // gcd(364, 1001) = 7 (R₆ and LCM_PRIMARY share factor 7)
        let g = TritInt::gcd(&TritInt::from_u64(364), &TritInt::from_u64(1001));
        assert_eq!(g.to_decimal(), 7);
    }
