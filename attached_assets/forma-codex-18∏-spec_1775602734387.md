...
**Pages are emergent.** Content that overflows Y generates new pages automatically. The user never declares page count. Pages only emerge after the grid's uniform scaling has been exhausted — only when further scaling would push any content cell below the 81 trit-pixel minimum.
...
### Minimum Cell Dimension: 3⁴ = 81 trit-pixels

For content cells (text, images, shapes). Structural cells (color swatches, rules, decorative spacing) allow 3³ = 27 trit-pixels.
...
## 3. Cell Modes — Water, Ice, Frost, Fire

Four cell modes determine how the container behaves relative to its content. The metaphor follows the Buried Question's water principle: content is water, the grid is the container.

| Mode | Metaphor | Container | Content | Grid Impact |
|------|----------|-----------|---------|-------------|
| Variable | Water | Grows/shrinks with content | Flows freely | Row/column heights adjust. May create pages after 81px limit. |
| Fixed | Ice | Locked size | Scrolls or clips internally | None — grid undisturbed. |
| Presentation | Frost | Locked size | Clips silently. No scrollbars. | None — grid undisturbed. |
| Plasma | Fire | Collapses to tab OR expands to overlay | Hidden when collapsed, visible when expanded | Collapsed: minimal. Expanded: overlay, no reflow. |
...
The cell has a locked size. Content that exceeds the boundary scrolls internally or clips (depending on overflow setting). The grid is undisturbed. Fixed cells do not contribute to row height calculation in rows with variable cells.

### 3.3 Presentation Mode — Frost

Content clips at the cell boundary. No scrollbars. No overflow indicator. The cell displays exactly what fits. For polished output where every cell shows a curated amount of content.

### 3.4 Plasma Mode — Fire

A plasma cell can collapse to a clickable tab (minimum 27×27 trit-pixels) and expand to occupy a configurable maximum area of the viewport, revealing hidden content.
...
Padding values are in trit-pixels, multiples of 3: 3, 9, 13, 27, 39, 81.
...
z=5    CELL LAYOUT
       ...
       • Inner padding { top, right, bottom, left } in trit-pixels
...
```javascript
{
  ...
  x: 3, y: 5,
  span: { cols: 1, rows: 1 },
  mode: 'variable',              // variable | fixed | presentation | plasma
...
Page bitmap at trit-pixel resolution is the print master. Scale = targetDPI / screenDPI × (printPageDimensions / viewportDimensions). Integer trit-pixels produce exact rational coordinates. Only live content (z=1, active face) is printed. Effect boundaries become page breaks.
...
z=1 content across all pages in reading order. All metadata lost. Comment header preserves grid dimensions:
...
| doLay / rLines / prep | Text engine unchanged. Scoped to cell. |
...
3. Port measureBlk/renderBlk into cell content area
4. Cell selection and markdown editing
5. Variable/Fixed/Presentation modes with row-height propagation
