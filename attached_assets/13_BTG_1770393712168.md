...
// Balanced ternary: 42 = P*27 + P*9 + N*3 + P*1 + Z = 1*27 + 1*9 + (-1)*3 + 1*1 + 0
...
// Unsigned: 42 = 1*27 + 1*9 + 2*3 + 0*1 = 1120₃
...
gateway.convert_file(input, output, FileFormat::TernaryC)?;

// Convert back
let ternary_file = File::open("data.tern")?;
let binary_file = File::create("data_restored.bin")?;

gateway.convert_file(ternary_file, binary_file, FileFormat::Binary)?;
```
...
```rust
// Arithmetic: Use Type A (balanced)
let value = gateway.i64_to_ternary_a(number);

// Indexing: Use Type B (unsigned)
let index = gateway.u64_to_ternary_b(position);

// Storage/Network: Use Type C (packed)
let packed = gateway.bytes_to_ternary_c(data);
```

### 2. Reuse Gateway Instances

```rust
// Good: Reuse gateway
let gateway = Gateway::new();
for value in values {
    let ternary = gateway.i64_to_ternary(value);
}

// Bad: Create new gateway each time
for value in values {
    let gateway = Gateway::new();  // Unnecessary allocation
    let ternary = gateway.i64_to_ternary(value);
}
```
...
let mut encoder = gateway.streaming_encoder(output);
for chunk in large_file.chunks(4096) {
    encoder.write_chunk(chunk)?;
}

// Bad: Load entire file into memory
let entire_file = std::fs::read("huge.bin")?;  // May OOM
let ternary = gateway.bytes_to_ternary(&entire_file);
```
...
- [TVM](./11_TVM.md) — Execute converted binary programs
- [Network Protocols](./10_NETWORK_PROTOCOLS.md) — Protocol bridging
- [Torsion Network](./09_TORSION_NETWORK.md) — Binary gateway nodes
