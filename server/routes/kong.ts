...
import { getKongServiceDefs, getKongServiceCatalog } from "@shared/service-catalog";
...
export function registerKongRoutes(app: Express, storage: IStorage): void {
        ...
        if (cpResp.ok) {
          ...
          for (const cp of (cpData.data || [])) {
            ...
            controlPlanes.push({
              ...
              routes: routeCount,
              configSynced: serviceCount >= getKongServiceDefs('').length
            });
          }
        }
      } catch {}
  ...
  function getPlenumnetServices() {
    ...
    return { baseUrl, services: getKongServiceDefs(baseUrl) };
  }
  ...
  app.get("/api/kong/service-catalog", async (_req, res) => {
    const baseUrl = 'https://plenumnet.replit.app';
    res.json(getKongServiceCatalog(baseUrl));
  });
