...
function loadTtcAddon(): TtcNativeAddon | null {
  ...
  for (const p of paths) {
    try {
      ...
      if (typeof addon.ttcCompress === 'function' && typeof addon.ttcDecompress === 'function') {
        ...
        console.log('[TTC] Native N-API addon loaded — TTC v5.0.3 engine active');
        return _ttcAddon;
      }
    } catch (e) {
  ...
  console.warn('[TTC] Native addon not found — using TTC v5.0.3 TypeScript fallback');
  return null;
}
