...
### Frontend
The frontend uses React, TypeScript, Tailwind CSS, Framer Motion, `shadcn/ui` for components, and Wouter for routing. It supports light and dark mode (class-based toggle with localStorage persistence) unified around a blue brand identity.

**Two-Layout Navigation Architecture** (added 2026-02-12):
- **MarketingLayout** (`client/src/App.tsx`): Used for public-facing pages (/, /about, /contact, /whitepaper, /terms, /privacy, /security, /aup). Features a `MarketingTopNav` component with mega dropdowns (Platform, Technology), standard dropdowns (Resources, Company), theme toggle, "Get Early Access" CTA, skip-nav link, mobile Sheet+Accordion menu, and hide-on-scroll-down behavior. Includes a shared `MarketingFooter` component.
- **DashboardLayout** (`client/src/App.tsx`): Used for tool/admin pages (/ternarydb, /api-demo, /hptp, /compression, /calendar, /13-moon, /docs, /compliance, /admin, /github, /kong-konnect). Features a collapsible shadcn Sidebar with resizable width, plus a compact header with sidebar trigger and theme toggle.
- **Sidebar Groups**: Tools (PlenumDB, API Explorer, HPTP Lab, Compression), Calendars (Universal Calendar, 13-Moon), Reference (Documentation, CNSA 2.0, Whitepaper), Admin (Dashboard, GitHub Manager, Kong Konnect — admin-only).
- **Dynamic Page Titles**: `usePageTitle` hook (`client/src/hooks/use-page-title.ts`) sets document.title per route.
- **Anchor Navigation**: Landing page sections have IDs (hero, platform, architecture, components, performance, markets, early-access). MarketingTopNav handles cross-page anchor scrolling via AnchorScrollContext.
