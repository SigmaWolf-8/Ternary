...
### Frontend
The frontend uses React, TypeScript, Tailwind CSS, Framer Motion, `shadcn/ui` for components, and Wouter for routing. It supports light and dark mode (class-based toggle with localStorage persistence) unified around a blue brand identity. Navigation uses a collapsible shadcn Sidebar. Key pages include the Landing Page, HPTP Timing API Demo, PlenumDB Product Page, Whitepaper Viewer, GitHub Manager, Kong Konnect Integration, Documentation Hub, CNSA 2.0 Compliance, and Admin Dashboard. Code splitting via React.lazy() is used for 12 non-critical pages, with an ErrorBoundary wrapping the app. The landing page includes: Hero with animated 5-layer architecture visual, Platform capabilities, Architecture diagram, Deployable components, Performance comparison table (with honesty rows), Trust signals, Developer code snippet with copy button, Target markets, and Developer CTA with "Apply for SDK Access" + "Book a Demo". Legal pages render markdown content with an inline parser instead of raw text. SEO includes robots.txt, sitemap.xml, manifest.json, and JSON-LD structured data. WCAG AA contrast compliance is verified.
