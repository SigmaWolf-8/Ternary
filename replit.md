...
The backend is built with Express.js and Node.js, using PostgreSQL and Drizzle ORM. It implements Unified Ternary Logic, Femtosecond Timing, and Phase Encryption (post-quantum, TL-Sponge-385-based GF(3) stream cipher with 364° geometric domain separation — IND-CPA secure via per-operation random nonce). Formal security analysis is in TM-2026-011 (`docs/proofs/Phase-Encryption-Security-Proof.md`): IND-CPA proof sketch via keyed-sponge PRF reduction (Gaži et al. 2015), INT-CTXT via sponge MAC, AE via Bellare-Namprempre composition, orthogonal security model formalization, differential/linear cryptanalysis bounds, and side-channel analysis. MAC verification uses `timingSafeEqual` and is mandatory on all nonce-based decryptions. The architecture includes microservices for payment processing and blockchain witnessing, a Femtosecond Timing Service, and a Certification Service. Security features include tiered rate limiting, CORS, Helmet.js, AES-256-GCM token encryption, input validation, hardened path sanitization, and API versioning.
