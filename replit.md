...
### Frontend
The frontend uses React, TypeScript, Tailwind CSS, Framer Motion, `shadcn/ui`, and Wouter, featuring light/dark modes and a single-layout navigation. Key pages include a Landing Page, About, Contact, HPTP Timing API Demo, PlenumDB Product Page, Whitepaper Viewer, GitHub Manager, Kong Konnect Integration, Documentation Hub, CNSA 2.0 Compliance, TSA Time-Stamping Authority, and an Admin Dashboard. It also includes a quantum-ternary simulator for interactive fault-tolerance simulations and FIPS 140-3 compliance checks. The homepage performance section showcases real measured benchmark data from `cargo run --release --bin salvi-bench`, including a 6-card benchmark grid, TL-DSA bar chart, and binary-vs-ternary comparison table. All benchmark constants live in `shared/constants.ts` as the single source of truth.
...
A digital notary service providing cryptographic proof-of-existence timestamps per RFC 3161. Features 4 TSA policies (COMPLY, FORENSICS, SENTINEL, SECURE) mapped from SecurityTier via `defaultTsaPolicy()` (Fortified→SENTINEL, Verified→COMPLY, Basic→null). Includes a Merkle tamper-evident audit log with cached root (invalidate-on-write, O(1) reads), dual-signature (RSA-4096 + TL-DSA-87 via direct in-process bridge at `server/crypto/tl-dsa-bridge.ts` — no HTTP loopback), HPTP timing integration, and ASN.1 wire protocol. TL-DSA keypair (128-byte sk / 64-byte pk, HMAC-SHA3-256 deterministic signatures) persists to `server/crypto/tsa-keys/tldsa-keypair.json` with 0600 permissions. Verification returns `tldsaPresent`, `tldsaVerified`, `tldsaAlgorithm`, `tldsaKeyId` via `verifyHex()` (proper verify, not re-sign). The frontend Documentation Hub (`/docs`) displays the full API reference: 279 endpoints across 23 services in 7 domain groups (Core Ternary Computing, Timing & Calendars, Cryptography & Trust, Security & Access Control, Network & Infrastructure, Signals & Applied Physics, Platform Operations), with domain-level filter pills, service subcategory drill-down, and live Try It panels for GET endpoints. It supports 9 endpoints for timestamping, verification, and auditing. Ternary Notary Stamp benchmark: TSA ~75ms, Hedera HCS ~1,800ms, verification ~105ms (TSA offline ~55ms + Hedera cached ~50ms). TSA JSON responses support `Accept: application/x-tern` for PlenumNET ternary compression (-32% vs JSON) and `compact: true` for tuple calendar encoding (-16%).

### plenum-stamp CLI (Go-to-Market)
Zero-dependency Node.js CLI at `cli/plenum-stamp/` for signing and verifying files with PlenumNET TSA timestamps. Commands: `sign <file>` (hash → TSA → .tsp + .tsp.json), `verify <file>` (validates .tsp token), `info <file.tsp>` (displays metadata), `cert` (downloads TSA certificate). Supports `--endpoint`, `--token`, `--algorithm`, `--format json|text`, `--compact`, `--calendars`. Publishable to npm as `plenum-stamp`. Companion GitHub Action at `.github/actions/plenum-stamp/action.yml` signs release artifacts in CI and uploads .tsp files as build artifacts or release assets.

### Hedera HCS Witnessing (Kong #22)
Blockchain-based non-repudiation via Hedera Consensus Service. Submits cryptographic witness hashes to an HCS topic for immutable, ordered, timestamped proof of PlenumNET operations. Includes 6 endpoints for submission, status lookup, and verification. Mirror node proof verification includes a 5-minute TTL in-memory cache (5,000 entries max, FIFO eviction) for hot-path lookups.
