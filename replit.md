...
The kernel boots as a bare-metal binary (`src/kernel/src/main.rs`) for three architectures: x86_64 (multiboot2 + 32→64 trampoline), aarch64 (QEMU virt, PL011 UART), and riscv64 (OpenSBI, SBI console). Uses a 1MB bump allocator for heap, per-arch serial drivers, and walks 11 `BootSequence` stages from the `arch/boot.rs` infrastructure. After boot, initializes PlenumBrowser (320×200 CPU framebuffer, renders test page), exercises the z=0 distributor, and prints sentinel `PLENUMNET KERNEL BOOT OK`. Linker scripts per-arch in `src/kernel/linker-{x86_64,aarch64,riscv64}.ld`, build.rs auto-selects the correct script for bare-metal targets. The `boot.sh` script builds and optionally launches QEMU with `--run` or `--ci` (smoke test with sentinel grep). CI workflow (`build-kernel.yml`) builds for bare-metal targets and runs QEMU smoke tests per architecture.
